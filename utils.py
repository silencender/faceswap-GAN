...
def showG(test_A, test_B, layout_A, layout_B, path_A, path_B, batchSize):
    figure_A = np.stack([
        test_A,
        layout_A,
        np.squeeze(np.array([path_A([test_A[i:i+1], layout_A[i:i+1]]) for i in range(test_A.shape[0])])),
        np.squeeze(np.array([path_B([test_A[i:i+1], layout_A[i:i+1]]) for i in range(test_A.shape[0])])),
        ], axis=1 )
    figure_B = np.stack([
        test_B,
        layout_B,
        np.squeeze(np.array([path_B([test_B[i:i+1], layout_B[i:i+1]]) for i in range(test_B.shape[0])])),
        np.squeeze(np.array([path_A([test_B[i:i+1], layout_B[i:i+1]]) for i in range(test_B.shape[0])])),
        ], axis=1 )
...
def showG_mask(test_A, test_B, layout_A, layout_B, path_A, path_B, batchSize):
    figure_A = np.stack([
        test_A,
        layout_A,
        (np.squeeze(np.array([path_A([test_A[i:i+1], layout_A[i:i+1]]) for i in range(test_A.shape[0])])))*2-1,
        (np.squeeze(np.array([path_B([test_A[i:i+1], layout_A[i:i+1]]) for i in range(test_A.shape[0])])))*2-1,
        ], axis=1 )
    figure_B = np.stack([
        test_B,
        layout_B,
        (np.squeeze(np.array([path_B([test_B[i:i+1], layout_B[i:i+1]]) for i in range(test_B.shape[0])])))*2-1,
        (np.squeeze(np.array([path_A([test_B[i:i+1], layout_B[i:i+1]]) for i in range(test_B.shape[0])])))*2-1,
        ], axis=1 )
